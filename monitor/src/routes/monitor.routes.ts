  ...
  app.get("/api/monitor", async (req: Request, res: Response) => {
    try {
      ...
      const services = {
        ...
        client: await checkServiceHealth("http://client:3000/api", "Client"),
...
async function checkServiceHealth(
  url: string,
  serviceName: string
): Promise<{ status: string; responseTime: number; message?: string }> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      logger.info(
        `${serviceName} 헬스체크 성공 (${responseTime}ms)`,
        JSON.stringify(data)
      );
      return {
